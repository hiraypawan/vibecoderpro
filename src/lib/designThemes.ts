...
export const DESIGN_THEMES: DesignTheme[] = [
  {
    ...
    motifs: ['Thick 3-4px black borders', 'Hard offset box-shadows (8px 8px 0 #000)', 'Numbered sections like "01 / 04"', 'Underlined links', 'Strikethrough on old prices', 'Marquee scrollers', 'Big rotated badges', 'Emoji or block characters as decoration'],
    ...
    classVocabulary: ['brutal-block', 'raw-button', 'expose-grid', 'hard-shadow', 'offset-card', 'numbered-section', 'thick-rule', 'rotate-badge'],
  },
  {
    id: 'glassmorphism',
    ...
    classVocabulary: ['glass-panel', 'frosted-card', 'orb', 'float-element', 'aurora-bg', 'blur-card', 'glow-hover', 'chip-tag'],
  },
  {
    id: 'editorial',
    ...
    classVocabulary: ['dropcap', 'pull-quote', 'byline', 'figure-caption', 'eyebrow', 'rule-divider', 'column-text', 'margin-note'],
  },
  {
    id: 'cyberpunk',
    name: 'Cyberpunk / Neon',
    ...
    classVocabulary: ['neon-glow', 'scanlines', 'hud-frame', 'glitch-text', 'chamfer-btn', 'hex-grid', 'corner-bracket', 'terminal-block'],
  },
  {
    id: 'swiss',
    ...
    fonts: { display: 'Inter', body: 'Inter', mono: 'JetBrains Mono', googleFontsUrl: GOOGLE(['Inter:wght@400;500;700;900', 'JetBrains+Mono:wght@400;700']) },
    ...
    classVocabulary: ['grid-cell', 'num-marker', 'rule-line', 'geo-shape', 'justify-text', 'micro-meta', 'asymmetric-block', 'numbered-note'],
  },
  {
    id: 'playful',
    ...
    classVocabulary: ['bento-card', 'tilted', 'squiggle', 'speech-bubble', 'stamp-badge', 'wobble', 'sticker', 'pop-in'],
  },
  {
    id: 'terminal',
    ...
    motifs: ['$ command prompt prefixes', 'Blinking cursor (animation: blink 1s step-end infinite)', 'ASCII art headers (using block characters)', 'Box-drawing characters for separators', 'Cat / Less / Grep command demos', 'Timestamp prefixes', 'Error messages styled as red', 'Success as green checkmark', 'Loading animations'],
    ...
    classVocabulary: ['term-prompt', 'ascii-divider', 'cmd-output', 'term-line', 'blinking-cursor', 'box-draw', 'cmd-success', 'cmd-error'],
  },
  {
    id: 'organic',
    name: 'Organic / Earthy',
    ...
    motifs: ['Blob SVG shapes behind content (border-radius: 30% 70% 70% 30%)', 'Hand-drawn line illustrations (use SVG)', 'Leaf, branch, dot patterns', 'Curved text along paths', 'Watercolor-style background gradients', 'Sticker-like circular badges'],
    ...
    classVocabulary: ['blob-bg', 'organic-card', 'leaf-pattern', 'watercolor', 'curved-text', 'circular-badge', 'gentle-fade', 'soft-shadow'],
  },
  {
    id: 'luxury',
    name: 'Luxury / Serif',
    ...
    motifs: ['Thin gold hairlines (1px solid #c9a961)', 'Drop caps in gold', 'Tracking: 0.3em on small caps eyebrows', 'Centered monogram logos', 'Engraved-looking dividers', 'Numbers in serif italic', 'All-caps for nav with letter-spacing'],
    ...
    classVocabulary: ['gold-rule', 'monogram', 'engraved-divider', 'serif-num', 'tracked-caps', 'parallax-slow', 'luxe-card', 'wax-seal'],
  },
  {
    id: 'bento',
    name: 'Bento Grid',
    ...
    fonts: { display: 'Inter', body: 'Inter', googleFontsUrl: GOOGLE(['Inter:wght@400;500;600;700;800']) },
    layout: 'Strict bento grid. Mix of 1x1, 1x2, 2x1, 2x2 cards. Tight gaps. Each card is self-contained.',
    motifs: ['Mixed-size cards in asymmetric grid', 'Each card has unique background color from a soft palette', 'Large numerical stats in some cards', 'Image+text pairs', 'Icon+label chips', 'Subtle inner shadows on cards'],
    ...
    classVocabulary: ['bento-cell', 'tile', 'stat-card', 'feature-tile', 'icon-chip', 'lift-hover', 'soft-pad', 'inner-glow'],
  },
  {
    id: 'retro',
    ...
    motifs: ['Sunset gradient (orange→pink→purple→navy)', 'Perspective grid floor in hero (CSS transform)', 'Horizontal-striped sun', 'Palm tree silhouettes (SVG)', 'Chrome text effect (gradient + text-shadow)', 'Scan lines overlay', 'VHS chromatic aberration', 'Pixelated cursor'],
    ...
    classVocabulary: ['sun-disc', 'grid-floor', 'chrome-text', 'scanline-overlay', 'neon-outline', 'palm-svg', 'pulse-glow', 'crt-flicker'],
  },
  {
    id: 'soft',
    ...
    motifs: ['Soft pastel gradient blobs in background', '16-24px border-radius on everything', 'Subtle gradient borders', 'Icon chips with colored backgrounds', 'Avatars with colored ring', 'Soft drop shadows: 0 4px 24px rgba(0,0,0,0.06)'],
    ...
    classVocabulary: ['pill-chip', 'soft-card', 'lavender-bg', 'gentle-lift', 'gradient-border', 'icon-chip', 'pastel-blob', 'soft-shadow'],
  },
];
...
export function pickTheme(requestHint?: string): DesignTheme {
  ...
  const matches = DESIGN_THEMES.filter((t) => {
    const keywords: Record<string, string[]> = {
      ...
      playful: ['playful', 'kids', 'fun', 'cartoon', 'bouncy', 'birthday'],
...
CLASS NAME VOCABULARY (use these as the base class names — invent similar ones in the same spirit):
${theme.classVocabulary.map((c) => `.${c}`).join(', ')}
